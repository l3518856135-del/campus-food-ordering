// ==================== 校园点餐 Pages Functions API ====================
// 部署在 Cloudflare Pages，与前端共用 *.pages.dev 域名，避免 workers.dev 被墙
...
async function createToken(userId: number, role: string, secret: string): Promise<string> {
  ...
  const payload = { userId, role, exp: Math.floor(Date.now() / 1000) + 7 * 24 * 3600 };
...
async function handleMenu(request: Request, env: Env): Promise<Response> {
...
async function handleOrders(request: Request, env: Env, path: string): Promise<Response> {
  if (request.method === 'GET') {
    ...
    if (auth.role === 'merchant') {
      query = 'SELECT * FROM orders ORDER BY created_at DESC LIMIT 200'; params = [];
    } else {
      query = 'SELECT * FROM orders WHERE customer_id = ? ORDER BY created_at DESC LIMIT 50'; params = [auth.userId];
    }
    const result = await env.DB.prepare(query).bind(...params).all();
    const orders = (result.results as Record<string, unknown>[]).map(o => ({ ...o, items: JSON.parse(o.items as string) }));
    return json({ orders });
  }
  ...
  if (request.method === 'POST') {
    const auth = await authRequired(request, env);
    if (auth instanceof Response) return auth;
    const body = await request.json() as { area: string; store: string; items: unknown[]; customerName: string; customerPhone: string; customerDorm: string; note: string };
  ...
  if (statusMatch && request.method === 'PUT') {
    ...
    const valid = ['pending', 'accepted', 'delivering', 'completed', 'cancelled'];
    if (!valid.includes(status)) return error('无效的状态');
    await env.DB.prepare('UPDATE orders SET status = ?, updated_at = datetime(\'now\') WHERE id = ?').bind(status, statusMatch[1]).run();
    return json({ success: true });
  }
  ...
  if (deleteMatch && request.method === 'DELETE') {
    const auth = await authRequired(request, env);
    if (auth instanceof Response) return auth;
    const order = await env.DB.prepare('SELECT * FROM orders WHERE id = ?').bind(deleteMatch[1]).first() as Record<string, unknown> | null;
    if (!order) return error('订单不存在');
    if (auth.role !== 'merchant' && order.customer_id !== auth.userId) return error('无权操作', 403);
    await env.DB.prepare('DELETE FROM orders WHERE id = ?').bind(deleteMatch[1]).run();
    return json({ success: true });
  }
...
async function handleAddresses(request: Request, env: Env, path: string): Promise<Response> {
  ...
  if (request.method === 'POST') {
    ...
    if (body.isDefault) await env.DB.prepare('UPDATE addresses SET is_default = 0 WHERE user_id = ?').bind(auth.userId).run();
  ...
  const m = path.match(/^\/api\/addresses\/(.+)$/);
  if (m && request.method === 'PUT') {
    ...
    await env.DB.prepare('UPDATE addresses SET name = ?, phone = ?, dorm = ?, is_default = ? WHERE id = ? AND user_id = ?')
      .bind(body.name, body.phone, body.dorm, body.isDefault ? 1 : 0, m[1], auth.userId).run();
    return json({ success: true });
  }
  if (m && request.method === 'DELETE') {
    const auth = await authRequired(request, env);
    if (auth instanceof Response) return auth;
    await env.DB.prepare('DELETE FROM addresses WHERE id = ? AND user_id = ?').bind(m[1], auth.userId).run();
    return json({ success: true });
  }
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    if (path === '/api/menu') return handleMenu(request, env);
